...
import java.util.TreeMap;
	...
	public class AerospaceApplicationFinal {
		static Map aeroplaneMap = new TreeMap<Integer, Aeroplane>();  //generic
		...
		private static void addAeroplane(Aeroplane aeroplane) {
			try {
			aeroplaneMap.put(aeroplane.aeroplaneId, aeroplane);    //Passing aeroplane object as value for future extension
		...
		private static void findAeroplane(int aeroplaneId) {
			Aeroplane aeroplane = (Aeroplane)aeroplaneMap.get(aeroplaneId);    // get() returns Object. So we downcast to Aeroplane type.
			if(aeroplane != null) {
		...
		private static void deleteAeroplane(int aeroplaneId) {
			...
			if(aeroplane != null) {
				aeroplaneMap.remove(aeroplaneId);            //map.remove(key, here id) deletes the entire record/ row/ object 
